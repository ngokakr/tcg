...
//		}
		...
		for(int i = 0; i < instantateItemCount; i++) {
			...
			var itemNode = item.gameObject.GetComponent<CardNodeScript> ();
			itemNode.nowIndex = i;
			itemNode.Delegate = DeckBox;
...
//			int index = int.Parse (go.name);
	...
	void Update()
	{
//		Debug.Log (string.Format ("anch:{0} diff:{1} itemScale{2}", anchoredPosition, diffPreFramePosition, itemScale));
